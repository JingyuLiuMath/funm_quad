...
%% save data
file_name = "./data/qcd/stability_qcd.mat";
if save_flag
    save(file_name, "results_table", "results_table_names", "check_records", ...
        "method_list", "num_test", "quad_tol", "stop_tol", "m", ...
        "max_restarts", "ada_max_restarts", "truncation_length", ...
        "max_num_quad_points", "sketching_mat_type", "sketching_size", ...
        "ada_sketching_size_control", "cond_tol");
end
...
function short_name = shorten_method_name(method_name, max_len)
name = char(method_name);
if length(name) <= max_len
    short_name = string(name);
else
    short_name = string([name(1:max_len-1), '.']);
end
end
